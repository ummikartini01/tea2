...
<head>
    ...
    <title>Admin Login Test - Teazy</title>
    <style>
        body {
...
<body>
    <div class='test-container'>
        ...
        <p class='subtitle'>Test admin authentication flow</p>";
        ...
        if (\Illuminate\Support\Facades\Hash::check($password, $user->password)) {
            ...
            if (\Illuminate\Support\Facades\Auth::attempt(['email' => $email, 'password' => $password])) {
                ...
                if ($authUser->role === 'admin') {
                    echo "<div class='status success'>✅ Should redirect to: /admin/dashboard</div>";
                    echo "<div class='status info'>";
                    echo "<a href='/tea2/admin/dashboard' class='btn'>Go to Admin Dashboard</a>";
                    echo "</div>";
                } else {
                    echo "<div class='status error'>❌ User is not admin</div>";
                    echo "<div class='status info'>Should redirect to: /dashboard</div>";
                    echo "<div class='status info'>";
...
foreach ($adminUsers as $admin) {
    echo "<div class='status info'>";
    echo "<strong>{$admin->email}</strong> ({$admin->name})<br>";
    echo "<small>ID: {$admin->id}, Role: {$admin->role}</small>";
    echo "</div>";
}
...
echo "<input type='email' name='email' value='ummikartini2004@gmail.com' style='width: 100%; padding: 8px; margin: 5px 0; border: 1px solid #ddd; border-radius: 4px;'></p>";
...
echo "<button type='submit' class='btn'>Test Admin Login</button>";
echo "</form>";
...
echo "<a href='/tea2/test-admin-login.php' class='btn'>Refresh This Page</a>";
